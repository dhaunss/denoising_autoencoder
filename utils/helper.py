...
def spherical_to_cartesian(zenith, azimuth):
    ...
    z = np.cos(zenith)
    if hasattr(zenith, '__len__') and hasattr(azimuth, '__len__') and zenith.size > 1:  # to catch array of one float
...
if __name__ == "__main__":
    import radiotools.helpers as hp
    n = 10000
    from radiotools.AERA import coordinates, signal_prediction
    stations = coordinates.get_stations_CRS()
    positions = np.array(list(stations.values()))
    positions_names = np.array(list(stations.keys()))
    ...
    positions2 = np.array(list(stations.values()))[~(de_mask & AERA24_mask)]
    plt.plot(positions2[..., 0], positions2[..., 1], "ob")
    positions2 = np.array(list(stations.values()))[~(de_mask)]
